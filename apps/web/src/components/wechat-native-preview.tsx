...
export function WechatNativePreview({
  ...
  const fatigueToneClass =
    previewMetrics.fatigueLevel === "empty"
      ...
      ? "border-[#d8b0b2] bg-[#fff4f1] text-[#8f3136]"
      : previewMetrics.fatigueLevel === "medium"
        ? "border-[#dcc8a6] bg-[#fff8eb] text-[#7d6430]"
        : "border-emerald-200 bg-emerald-50 text-emerald-700";

  return (
    <div className="grid gap-6 bg-[radial-gradient(circle_at_top,rgba(196,138,58,0.12),transparent_32%),linear-gradient(180deg,#f5efe2_0%,#f8f5ee_100%)] p-6 xl:grid-cols-[260px_minmax(0,1fr)]">
      <div className="space-y-4">
        <div className="border border-stone-300/70 bg-white/80 p-4 shadow-ink">
          <div className="text-xs uppercase tracking-[0.2em] text-stone-500">微信原生预览</div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <div className="border border-stone-300/70 bg-white/80 p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-stone-500">估算屏数</div>
          ...
          <div className="border border-stone-300/70 bg-white/80 p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-stone-500">段落 / 配图</div>
            <div className="mt-2 font-serifCn text-3xl text-ink text-balance">
              {previewMetrics.paragraphCount} / {previewMetrics.imageCount}
            </div>
            <div className="mt-1 text-xs leading-6 text-stone-500">段落过密且配图偏少时，更容易出现划屏疲劳。</div>
          </div>
          <div className="border border-stone-300/70 bg-white/80 p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-stone-500">内容骨架</div>
            <div className="mt-2 text-sm leading-7 text-stone-700">
      ...
      <div className="flex justify-center">
        <div className="relative flex h-[812px] w-[375px] shrink-0 flex-col overflow-hidden rounded-[44px] border-[10px] border-stone-900 bg-[#f6f6f6] shadow-[0_30px_80px_rgba(36,28,18,0.28)]">
          <div className="absolute left-1/2 top-0 z-50 h-[32px] w-[154px] -translate-x-1/2 rounded-b-[22px] bg-stone-900" />
          <div className="flex h-9 shrink-0 items-end justify-between bg-stone-900 px-7 pb-1 pt-2 text-[11px] text-white">
            <span>9:41</span>
          ...
          <div className="relative z-40 flex h-14 shrink-0 items-center justify-between border-b border-[#d8d8d8] bg-[#ededed] px-4">
            ...
            <div className="text-[17px] font-medium text-stone-900">{accountName}</div>
            <div className="flex items-center gap-2 text-stone-700">
              <div className="flex h-7 w-16 items-center justify-between rounded-full border border-stone-300 bg-white px-2">
                <div className="h-1 w-1 rounded-full bg-stone-900" />
                <div className="h-1 w-1 rounded-full bg-stone-900" />
                <div className="h-1 w-1 rounded-full bg-stone-900" />
                <div className="mx-1 h-3 w-px bg-stone-300" />
                <div className="h-3 w-3 rounded-full border border-stone-900" />
              </div>
            </div>
          </div>

          <div ref={containerRef} className="flex-1 overflow-x-hidden overflow-y-auto bg-white pb-8">
            <div className="p-4">
              <h1 className="text-[23px] font-bold leading-[1.45] text-[#2c2c2c] text-balance">{title}</h1>
              <div className="mt-4 flex items-center justify-between gap-3 text-[14px] leading-6">
                <div>
                  <span className="text-[#576b95]">{accountName}</span>
                  <span className="ml-2 text-[#888]">{previewDate}</span>
                </div>
                <span className="text-[#b0b0b0]">{authorName}</span>
              </div>

              {previewMetrics.hasRenderableContent ? (
                <div
                  className="wechat-content-body mt-5 break-words text-[17px] leading-[1.8] text-[#2f2f2f]"
                  dangerouslySetInnerHTML={{ __html: html }}
                />
              ) : (
                <div className="mt-6 overflow-hidden rounded-[24px] border border-[#e6dfd3] bg-[linear-gradient(180deg,#fdfbf6_0%,#f7f2e8_100%)] px-5 py-6">
                  <div className="text-[12px] uppercase tracking-[0.2em] text-[#8d7a61]">真机占位态</div>
                  <div className="mt-4 font-serifCn text-[24px] leading-[1.5] text-[#2f2f2f]">这块屏幕会在你写下首段后，开始呈现真正的阅读体感。</div>
                  <div className="mt-3 text-[15px] leading-7 text-[#5d5549]">
                  ...
                  <div className="mt-5 space-y-3">
                    {[
                      ...
                      <div key={tip} className="border border-[#e6dfd3] bg-white/80 px-4 py-3 text-[13px] leading-6 text-[#6b6255]">
                        {tip}
                      </div>
                    ))}
                  </div>
              ...
              <div className="mt-8 rounded-[20px] bg-[#f7f7f7] px-4 py-3 text-[13px] leading-6 text-[#666]">
                上文为预估发布效果，真实渲染仍以当前模板和微信稿箱接收结果为准。
              </div>

              <div className="mt-8 flex items-center justify-between text-[14px] text-[#999]">
                <div className="flex items-center gap-4">
                  <span>阅读 {readCountStr}</span>
                  <span className="text-[#576b95]">分享</span>
              ...
              <div className="mt-6 border-t border-[#ececec] pt-4 text-[12px] leading-6 text-[#7f7f7f]">
                <div>喜欢作者</div>
                <div className="mt-2 flex items-center justify-between">
                  <span>{authorName}</span>
                  <button type="button" className="rounded-full bg-[#07c160] px-4 py-1.5 text-white">
                    关注
                  </button>
                </div>
              </div>
          ...
          <div className="flex h-16 shrink-0 items-center justify-around border-t border-[#dadada] bg-[#f6f6f6] text-[11px] text-stone-500">
            <div className="text-center">
              <div className="text-lg text-stone-700">◦</div>
              <div>微信</div>
            </div>
            <div className="text-center">
              <div className="text-lg text-stone-700">◦</div>
            ...
            <div className="text-center text-[#07c160]">
