...
import { ArrowRight, Bot, Image, Link2, RefreshCcw, Sparkles, Wand2 } from "lucide-react";
    ...
    <section className="border border-stone-300/40 bg-white shadow-ink">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {[
              ["《行业在降价，利润在出血》", "活字实验室", "2026-04-12 09:42", "推送成功", "在微信中预览"],
              ["《你为什么还在写机器腔调》", "商业评论札记", "2026-04-11 22:16", "Token 失效", "重新授权并重试"],
              ["《坏语言是如何粉饰现实的》", "活字实验室", "2026-04-10 18:03", "推送成功", "查看草稿箱"],
            ].map((row) => (
              <tr key={row[0]} className="border-t border-stone-200/80">
                <td className="px-6 py-4">{row[0]}</td>
                <td className="px-6 py-4 text-stone-600">{row[1]}</td>
                <td className="px-6 py-4 text-stone-600">{row[2]}</td>
                <td className="px-6 py-4">
                  <span className={row[3] === "推送成功" ? "text-emerald-600" : "text-cinnabar"}>{row[3]}</span>
                </td>
                <td className="px-6 py-4 text-cinnabar">{row[4]}</td>
              </tr>
            ))}
          </tbody>
        </table>
