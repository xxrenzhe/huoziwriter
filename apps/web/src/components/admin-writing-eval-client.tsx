...
export function AdminWritingEvalClient({
  ...
  const selectedRunSourceScheduleHref = selectedRunSourceSchedule ? buildAdminWritingEvalRunsHref({ scheduleId: selectedRunSourceSchedule.id }) : null;
  ...
  async function handleRetryRun() {
    ...
    setMessage("");
    const response = await fetch(`/api/admin/writing-eval/runs/${selectedRunId}/retry`, {
      method: "POST",
    });
    const json = await response.json();
    if (!response.ok || !json.success) {
  ...
  async function handlePromoteRun() {
    if (!selectedRunId) return;
    setMessage("");
    const response = await fetch(`/api/admin/writing-eval/runs/${selectedRunId}/promote`, {
      method: "POST",
    });
    const json = await response.json();
    if (!response.ok || !json.success) {
  ...
  async function handleDiscardRun() {
    if (!selectedRunId) return;
    setMessage("");
    const response = await fetch(`/api/admin/writing-eval/runs/${selectedRunId}/discard`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
        ...
        <form onSubmit={handleCreateScoringProfile} className={uiPrimitives.adminPanel + " space-y-3 p-5"}>
          ...
          <input value={scoringProfileForm.code} onChange={(event) => setScoringProfileForm((prev) => ({ ...prev, code: event.target.value }))} placeholder="编码，例如 viral-55-default" className={uiPrimitives.adminInput} />
          <input value={scoringProfileForm.name} onChange={(event) => setScoringProfileForm((prev) => ({ ...prev, name: event.target.value }))} placeholder="名称" className={uiPrimitives.adminInput} />
          ...
          <textarea value={scoringProfileForm.config} onChange={(event) => setScoringProfileForm((prev) => ({ ...prev, config: event.target.value }))} className={`min-h-[220px] ${uiPrimitives.adminInput}`} placeholder="评分画像配置 JSON" />
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
        <form onSubmit={handleCreateRun} className={uiPrimitives.adminPanel + " space-y-3 p-5"}>
          ...
          <select value={runForm.baseVersionRef} onChange={(event) => setRunForm((prev) => ({ ...prev, baseVersionRef: event.target.value }))} className={uiPrimitives.adminSelect}>
          ...
          <select value={runForm.candidateVersionRef} onChange={(event) => setRunForm((prev) => ({ ...prev, candidateVersionRef: event.target.value }))} className={uiPrimitives.adminSelect}>
          ...
          <select value={runForm.triggerMode} onChange={(event) => setRunForm((prev) => ({ ...prev, triggerMode: event.target.value }))} className={uiPrimitives.adminSelect}>
          ...
          <textarea value={runForm.summary} onChange={(event) => setRunForm((prev) => ({ ...prev, summary: event.target.value }))} className={`min-h-[100px] ${uiPrimitives.adminInput}`} placeholder="实验摘要，可选" />
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
        <form onSubmit={handleCreateSchedule} className={uiPrimitives.adminPanel + " space-y-3 p-5"}>
          <div className="flex items-center justify-between gap-3">
            <div>
      ...
      <section className={uiPrimitives.adminPanel + " p-5"}>
        ...
        {selectedRunDetail ? (
          <div className="mt-6 space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={handleRetryRun} className={uiPrimitives.adminSecondaryButton}>
                重跑当前实验
              </button>
              <button
                type="button"
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
              ...
              <form onSubmit={handleCreateFeedback} className="border border-stone-800 bg-stone-950 px-4 py-4">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  />
                  <div className="grid gap-3 md:grid-cols-2">
                    <input value={feedbackForm.openRate} onChange={(event) => setFeedbackForm((prev) => ({ ...prev, openRate: event.target.value }))} placeholder="打开率 %" className={uiPrimitives.adminInput} />
                    <input
                      ...
                      className={uiPrimitives.adminInput}
                    />
                    <input value={feedbackForm.shareRate} onChange={(event) => setFeedbackForm((prev) => ({ ...prev, shareRate: event.target.value }))} placeholder="分享率 %" className={uiPrimitives.adminInput} />
                    <input
                      ...
                      className={uiPrimitives.adminInput}
                    />
                    <input value={feedbackForm.readCount} onChange={(event) => setFeedbackForm((prev) => ({ ...prev, readCount: event.target.value }))} placeholder="阅读量" className={uiPrimitives.adminInput} />
                    <input value={feedbackForm.likeCount} onChange={(event) => setFeedbackForm((prev) => ({ ...prev, likeCount: event.target.value }))} placeholder="点赞数" className={uiPrimitives.adminInput} />
                    <input value={feedbackForm.commentCount} onChange={(event) => setFeedbackForm((prev) => ({ ...prev, commentCount: event.target.value }))} placeholder="评论数" className={uiPrimitives.adminInput} />
                    <input
                      type="datetime-local"
