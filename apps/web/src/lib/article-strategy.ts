...
function countSnapshotSignals(value: string) {
  let score = 0;
  if (/(周[一二三四五六日天]|早上|上午|中午|下午|晚上|凌晨|\d{1,2}\s*点|昨天|今天|那天|当时)/.test(value)) score += 1;
  if (/(办公室|工位|地铁|电梯|会议室|家里|咖啡馆|手机|微信|群里|现场|门口)/.test(value)) score += 1;
  if (/(说|回|盯着|点开|弹出|走进|坐着|改|发来|看到|愣住|停住|想了|忍不住)/.test(value)) score += 1;
  if (/(觉得|怀疑|犹豫|委屈|尴尬|兴奋|别扭|上头|烦|怕|不甘|不对劲)/.test(value)) score += 1;
  return score;
}
