...
import { isStandardPlanCode } from "./plan-entitlements";

const MANAGED_ACCOUNT_ROLES = new Set(["admin", "user"]);
const SUBSCRIPTION_STATUSES = new Set(["active", "inactive", "ended"]);

export function parseManagedRole(value: unknown, fallback: string) {
...
export function parsePlanDraft(body: Record<string, unknown>) {
  ...
  if (!isStandardPlanCode(code)) {
    throw new Error("套餐 code 只允许为 free、pro 或 ultra");
  }
  if (!name) {
  ...
  return {
    ...
    languageGuardRuleLimit: body.languageGuardRuleLimit ?? null,
