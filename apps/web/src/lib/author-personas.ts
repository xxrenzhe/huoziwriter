...
export type AuthorPersonaRecord = {
  id: number;
  user_id: number;
  name: string;
...
type AuthorPersonaSourceAnalysis = {
  summary: string;
  identityTags: string[];
...
type AuthorPersonaAnalysisSourceInput = {
...
export function getAuthorPersonaLimit(planCode: "free" | "pro" | "ultra") {
  if (planCode === "pro") return 3;
  if (planCode === "ultra") return 10;
  return 1;
}

export async function getAuthorPersonaLimitForUser(userId: number) {
  const { effectivePlanCode } = await getUserPlanContext(userId);
  return getAuthorPersonaLimit(effectivePlanCode);
}

export async function getAuthorPersonas(userId: number) {
  await ensureExtendedProductSchema();
  const db = getDatabase();
  const rows = await db.query<AuthorPersonaRecord>(
  ...
  const profileIds = Array.from(new Set(rows.map((row) => row.bound_writing_style_profile_id).filter((item): item is number => typeof item === "number" && item > 0)));
...
async function countAuthorPersonas(userId: number) {
  ...
  const row = await db.queryOne<{ count: number }>(
    "SELECT COUNT(*) as count FROM author_personas WHERE user_id = ?",
    [userId],
  );
  return row?.count ?? 0;
}
...
async function getAuthorPersonaRow(userId: number, personaId: number) {
  const db = getDatabase();
  return db.queryOne<AuthorPersonaRecord>(
    `SELECT id, user_id, name, identity_tags_json, writing_style_tags_json, bound_writing_style_profile_id,
            summary, domain_keywords_json, argument_preferences_json, tone_constraints_json, audience_hints_json,
            source_mode, is_default, created_at, updated_at
     FROM author_personas
...
export async function createAuthorPersona(input: {
  userId: number;
  name?: string | null;
  ...
  const name = String(input.name || "").trim() || deriveAuthorPersonaName(identityTags, writingStyleTags);
  ...
  if (shouldBeDefault) {
    await db.exec("UPDATE author_personas SET is_default = ?, updated_at = ? WHERE user_id = ?", [false, now, input.userId]);
  }
  ...
  const created = await getAuthorPersonas(input.userId);
...
export async function updateAuthorPersona(input: {
  userId: number;
  personaId: number;
  name?: string | null;
  identityTags?: unknown;
  ...
  await ensureExtendedProductSchema();
  const current = await getAuthorPersonaRow(input.userId, input.personaId);
  if (!current) {
  ...
  const domainKeywords = input.domainKeywords === undefined ? parseTagList(current.domain_keywords_json) : normalizeOptionalStringList(input.domainKeywords, 8);
  const argumentPreferences = input.argumentPreferences === undefined ? parseTagList(current.argument_preferences_json) : normalizeOptionalStringList(input.argumentPreferences, 6);
  const toneConstraints = input.toneConstraints === undefined ? parseTagList(current.tone_constraints_json) : normalizeOptionalStringList(input.toneConstraints, 6);
  const audienceHints = input.audienceHints === undefined ? parseTagList(current.audience_hints_json) : normalizeOptionalStringList(input.audienceHints, 6);
  const sourceMode = input.sourceMode === undefined ? current.source_mode : String(input.sourceMode || "").trim() === "analyzed" ? "analyzed" : "manual";
  const name = String(input.name ?? current.name).trim() || deriveAuthorPersonaName(identityTags, writingStyleTags);
  const now = new Date().toISOString();
  const db = getDatabase();
  ...
  const updated = (await getAuthorPersonas(input.userId)).find((item) => item.id === input.personaId);
  if (!updated) {
...
async function analyzeAuthorPersonaSource(input: {
  name?: string | null;
  ...
  const systemPrompt = [
    "你是作者人设分析器。",
    "请根据用户提供的资料，总结出一个适合中文内容写作系统的人设画像。",
    "只返回 JSON，不要输出任何额外解释。",
  ].join("\n");
  const userPrompt = [
  ...
  return {
    ...
    suggestedName:
      String(parsed.suggestedName || "").trim() || String(input.name || "").trim() || deriveAuthorPersonaName(identityTags, writingStyleTags),
    rawAnalysis: parsed,
  } satisfies AuthorPersonaSourceAnalysis;
}

export async function createAuthorPersonaFromSourceAnalysis(input: {
  userId: number;
  name?: string | null;
  sources: AuthorPersonaAnalysisSourceInput[];
  isDefault?: boolean;
  ...
  const analysis = await analyzeAuthorPersonaSource({
    name: input.name,
  ...
  for (const source of normalizedSources) {
    await db.exec(
      `INSERT INTO author_persona_sources (
  ...
  const created = (await getAuthorPersonas(input.userId)).find((item) => item.id === persona.id);
  if (!created) {
...
export async function deleteAuthorPersona(userId: number, personaId: number) {
  await ensureExtendedProductSchema();
  const db = getDatabase();
  const current = await getAuthorPersonaRow(userId, personaId);
  if (!current) {
    throw new Error("作者人设不存在");
  }

  const total = await countAuthorPersonas(userId);
  if (total <= 1) {
    throw new Error("至少保留 1 个作者人设");
  }

  await db.exec("DELETE FROM author_personas WHERE id = ? AND user_id = ?", [personaId, userId]);

  if (Boolean(current.is_default)) {
    const next = await db.queryOne<{ id: number }>(
      "SELECT id FROM author_personas WHERE user_id = ? ORDER BY id ASC LIMIT 1",
      [userId],
    );
    if (next) {
      await db.exec(
        "UPDATE author_personas SET is_default = ?, updated_at = ? WHERE id = ? AND user_id = ?",
        [true, new Date().toISOString(), next.id, userId],
      );
    }
  }
}
