...
import { ensureColumn, hasTable as hasTableCompat } from "./schema-bootstrap-db-utils";
...
async function applySpecialCaseMigration(type: DatabaseType, fileName: string) {
  if (type !== "sqlite" || fileName !== "002_ai_call_observation_article_context.sqlite.sql") {
    return false;
  }
  const db = getDatabase();
  if (!(await hasTableCompat("ai_call_observations"))) {
    await db.exec(`
    ...
    await ensureColumn("ai_call_observations", "article_id", "INTEGER");
  }
  await db.exec(
...
export async function runPendingMigrations() {
  ...
  for (const fileName of files) {
    ...
    if (await applySpecialCaseMigration(type, fileName)) {
      await markMigrationApplied(type, fileName);
