...
test("resumeArticleAutomationRun completes strategyOnly runs and skips later stages", async () => {
  await withTempDatabase("strategy-only", async () => {
    ...
    const recommendedTitle = String(getRecord(detail?.stages.find((item) => item.stageCode === "titleOptimization")?.outputJson)?.recommendedTitle || "").trim();
    assert.ok(recommendedTitle.length > 0);
    assert.equal(detail?.article?.title, recommendedTitle);
...
test("resumeArticleAutomationRun completes draftPreview runs end-to-end", async () => {
  await withTempDatabase("draft-preview", async () => {
    ...
    assert.equal(getRecord(detail?.stages.find((item) => item.stageCode === "titleOptimization")?.qualityJson)?.reusedFromOutlinePlanning, false);
    assert.equal(getRecord(detail?.stages.find((item) => item.stageCode === "openingOptimization")?.qualityJson)?.reusedFromOutlinePlanning, false);
    ...
    assert.ok(recommendedTitle.length > 0);
    assert.equal(detail?.article?.title, recommendedTitle);
    const publishGuardOutput = getRecord(detail?.stages.find((item) => item.stageCode === "publishGuard")?.outputJson);
    assert.ok(Array.isArray(publishGuardOutput?.methodologyGates));
    assert.equal((publishGuardOutput?.methodologyGates as unknown[]).length, 6);
