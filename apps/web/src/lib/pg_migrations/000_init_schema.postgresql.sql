...
INSERT INTO prompt_versions (
  ...
  ('language_guard_audit', 'v1.1.0', 'review', '语言守卫审校', '按真实发布阻塞项清理禁用表达、模板句和过长句', 'system:review', 'languageGuardAudit', '你是公众号终审编辑，只负责把已成稿改到可发布，不负责重写选题。必须输出修复后的完整 Markdown 正文，不要解释。优先级：先删禁用表达和模板句，再拆影响阅读的长句，最后打散过于工整的段落呼吸。禁用表达必须改成具体动作、对象、结果或代价；不能只换同义抽象词。长句拆分不得新增事实，不得扩大证据含义，不得改变核心判断。保留标题层级、证据、引用和已核查事实。', 'zh-CN', TRUE, '补强发布阻塞项定向修复，避免只做泛化润色'),
...
INSERT INTO topic_sources (name, homepage_url, source_type, topic_verticals_json, priority, is_active) VALUES
  ('Hacker News Top Stories', 'https://hacker-news.firebaseio.com/v0/topstories.json', 'news', '["ai_products"]'::jsonb, 95, TRUE),
  ('Hacker News Jobs', 'https://hacker-news.firebaseio.com/v0/jobstories.json', 'news', '["career","overseas_income"]'::jsonb, 93, TRUE),
  ('V2EX Hot Topics', 'https://www.v2ex.com/api/topics/hot.json', 'community', '["ai_products","career","side_hustles","overseas_income"]'::jsonb, 94, TRUE),
  ('Remotive Remote Jobs', 'https://remotive.com/api/remote-jobs', 'news', '["career","overseas_income","side_hustles"]'::jsonb, 92, TRUE),
  ('Side Hustle Nation Feed', 'https://www.sidehustlenation.com/feed/', 'rss', '["side_hustles","overseas_income"]'::jsonb, 91, TRUE),
  ('Location Rebel Feed', 'https://www.locationrebel.com/feed/', 'rss', '["overseas_income","side_hustles","career"]'::jsonb, 90, TRUE),
  ('Ahrefs Blog Feed', 'https://ahrefs.com/blog/feed/', 'rss', '["affiliate_marketing","ai_products"]'::jsonb, 89, TRUE),
  ('Backlinko Feed', 'https://backlinko.com/feed', 'rss', '["affiliate_marketing"]'::jsonb, 88, TRUE),
  ('Niche Pursuits Feed', 'https://www.nichepursuits.com/feed/', 'rss', '["affiliate_marketing","side_hustles","overseas_income"]'::jsonb, 87, TRUE),
  ('Social Media Examiner Feed', 'https://www.socialmediaexaminer.com/feed/', 'rss', '["affiliate_marketing","side_hustles"]'::jsonb, 86, TRUE),
  ('HubSpot Marketing Feed', 'https://blog.hubspot.com/marketing/rss.xml', 'rss', '["affiliate_marketing"]'::jsonb, 85, TRUE),
  ('Lenny''s Newsletter Feed', 'https://www.lennysnewsletter.com/feed', 'rss', '["ai_products","career"]'::jsonb, 84, TRUE),
