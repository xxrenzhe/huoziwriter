...
import { getBannedWords, getDocumentById, getFragmentsByUser } from "@/lib/repositories";
...
function buildCommands(markdownContent: string, bannedWords: string[]) {
  const commands = [
  ...
  if (bannedWords.some((word) => markdownContent.includes(word))) {
    commands.unshift("扫描并替换当前命中的死刑词，保留事实信息");
  }
...
export default async function CommandPage({ params }: CommandPageProps) {
  ...
  const [document, fragments, bannedWords] = await Promise.all([
  ...
  const bannedWordList = bannedWords.map((item) => item.word);
  const suggestedCommands = buildCommands(document.markdown_content, bannedWordList);
      ...
      <div className="border border-stone-300/40 bg-white p-6 shadow-ink">
        ...
        <div className="mt-6 border border-stone-300/40 bg-[#faf7f0] p-5">
          ...
          <p className="mt-3 text-sm leading-7 text-stone-700">
            当前状态：{document.status}。正文长度约 {document.markdown_content.length} 字符，已挂载碎片 {latestFragments.length} 条，专属死刑词 {bannedWordList.length} 个。
          </p>
        </div>
        <div className="mt-6">
        ...
        <div className="mt-6 rounded-none border border-dashed border-stone-300 bg-[#fffaf2] p-5 text-sm leading-7 text-stone-700">
          命令会复用当前文稿、最近碎片、死刑词和排版基因一起做整篇改写。每次执行前系统都会先留一份快照，便于回滚。
        </div>
      </div>
