import { requireAdminAccess, syncUserSubscription, toStoredManagedUserRole } from "@/lib/auth";
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const nextRole = parseManagedRole(body.role, current.role);
    ...
    await db.exec(
      `UPDATE users
       ...
       WHERE id = ?`,
      [
        toStoredManagedUserRole(nextRole as "admin" | "user"),
        nextPlanCode,
