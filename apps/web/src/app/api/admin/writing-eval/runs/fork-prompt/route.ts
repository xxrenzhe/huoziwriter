...
export async function POST(request: Request) {
  try {
    ...
    if (!isPromptBackedVersionType(baseVersionType)) {
      return fail("当前仅支持从 prompt_version、fact_check、title_template 或 lead_template 一键 fork 候选并发起实验", 400);
    }
    const { promptId, version } = parsePromptVersionRef(body.baseVersionRef);
    ...
    const run = await createWritingEvalRun({
      ...
      baseVersionType,
      baseVersionRef: String(body.baseVersionRef || "").trim(),
      ...
      summary: [String(body.summary || "").trim(), `auto-fork:${candidate.promptVersionRef}`].filter(Boolean).join("\n"),
    ...
    return fail(error instanceof Error ? error.message : "创建候选 Prompt 并发起实验失败", 400);
  }
}
