...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const workflow = await getDocumentWorkflow(Number(params.id), session.userId);
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const workflow = await setDocumentWorkflowPendingPublishIntent({
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const workflow = await clearDocumentWorkflowPendingPublishIntent({
      documentId: Number(params.id),
      userId: session.userId,
    });
    return ok({ pendingPublishIntent: workflow.pendingPublishIntent ?? null, cleared: true });
  } catch (error) {
