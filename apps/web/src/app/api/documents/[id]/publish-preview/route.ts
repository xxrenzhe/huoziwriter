...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const title = typeof body.title === "string" && body.title.trim() ? body.title.trim() : document.title;
    const markdownContent =
      typeof body.markdownContent === "string" ? body.markdownContent : document.markdown_content;
    const templateId =
      body.templateId === null
        ? null
        : typeof body.templateId === "string" && body.templateId.trim()
          ? body.templateId.trim()
          : document.wechat_template_id;
    ...
    const savedHtml = document.html_content || "";
    ...
    if (markdownContent !== document.markdown_content) {
      mismatchWarnings.push("当前正文尚未保存，正式发布前会先保存正文并重新渲染。");
    }
    if ((templateId || null) !== (document.wechat_template_id || null)) {
